...
export const CustomerUploader: React.FC = () => {
  ...
  // Table columns
  const columns = React.useMemo<ColumnDef<CustomerRow, any>[]>(
    () => [
      ...
      { accessorKey: 'linkedinurl', header: 'LinkedIn URL' },
      { accessorKey: 'facebookurl', header: 'Facebook URL' },
      { accessorKey: 'twitterhandle', header: 'Twitter' },
      { accessorKey: 'notes', header: 'Notes' },
      { accessorKey: 'extradata', header: 'Extra Data', cell: ({ getValue }) => (getValue() ? JSON.stringify(getValue()) : '') },
          ...
          if (Array.isArray(data)) {
            allCustomers = allCustomers.concat(
              data.map((customer: any) => ({
                ...
                twitterhandle: customer.twitterHandle ?? null,
                notes: customer.notes ?? null,
                extradata: customer.extraData ?? null,
  ...
  const handleExportCSV = () => {
    ...
    saveAs(blob, 'flattened_customers.csv');
  };
  const handleExportXLSX = () => {
    ...
    saveAs(blob, 'flattened_customers.xlsx');
  };
  ...
  return (
    <div className="flex flex-col gap-4 p-4 border rounded-md bg-white shadow w-full max-w-5xl mx-auto mt-8">
      <h2 className="text-xl font-bold mb-2">Upload Customers (Teamwork Desk)</h2>
      <div className="flex gap-4 items-center">
        ...
        <Button onClick={handleExportCSV} variant="outline" disabled={customers.length === 0}>Export CSV</Button>
        <Button onClick={handleExportXLSX} variant="outline" disabled={customers.length === 0}>Export XLSX</Button>
        <Button onClick={handleInsert} variant="default" disabled={uploading || customers.length === 0}>
          {uploading ? 'Uploading...' : 'Insert to Supabase'}
        </Button>
      </div>
      <div className="flex items-center gap-4 text-sm">
        <span>Rows: <b>{customers.length}</b></span>
        {results.length > 0 && (
        ...
        <table className="min-w-full text-xs">
          ...
          <tbody>
            {table.getRowModel().rows.length ? (
              ...
              <tr><td colSpan={columns.length} className="text-center py-4">No customers loaded.</td></tr>
            )}
          </tbody>
        </table>
      </div>
