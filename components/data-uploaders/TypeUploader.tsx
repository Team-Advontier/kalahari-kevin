...
export const TypeUploader: React.FC = () => {
  ...
  const columns = React.useMemo<ColumnDef<TypeRow, any>[]>(
    () => [
      ...
      { accessorKey: 'inboxes', header: 'Inboxes', cell: ({ getValue }) => (getValue() ? JSON.stringify(getValue()) : '') },
      {
          ...
          if (Array.isArray(data)) {
            allTypes = allTypes.concat(
              data.map((type: any) => ({
                typeid: type.id,
                name: type.name ?? null,
                displayorder: type.displayOrder ?? null,
                isdefault: type.default ?? null,
                enabledforfutureinboxes: type.enabledForFutureInboxes ?? null,
                createdat: type.createdAt ?? null,
                updatedat: type.updatedAt ?? null,
                createdbyid: type.createdBy?.id ?? null,
                createdbytype: type.createdBy?.type ?? null,
                updatedbyid: type.updatedBy?.id ?? null,
                updatedbytype: type.updatedBy?.type ?? null,
                state: type.state ?? null,
                inboxes: type.inboxes ?? null,
                source_data: type,
  ...
  const handleExportCSV = () => {
    const csv = Papa.unparse(types);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, 'flattened_types.csv');
  };
  const handleExportXLSX = () => {
    ...
    saveAs(blob, 'flattened_types.xlsx');
  };
  ...
  return (
    <div className="flex flex-col gap-4 p-4 border rounded-md bg-white shadow w-full max-w-5xl mx-auto mt-8">
      <h2 className="text-xl font-bold mb-2">Upload Ticket Types (Teamwork Desk)</h2>
      <div className="flex gap-4 items-center">
        ...
        <Button onClick={handleExportCSV} variant="outline" disabled={types.length === 0}>Export CSV</Button>
        <Button onClick={handleExportXLSX} variant="outline" disabled={types.length === 0}>Export XLSX</Button>
        <Button onClick={handleInsert} variant="default" disabled={uploading || types.length === 0}>
          {uploading ? 'Uploading...' : 'Insert to Supabase'}
        </Button>
      </div>
      <div className="flex items-center gap-4 text-sm">
        <span>Rows: <b>{types.length}</b></span>
        {results.length > 0 && (
        ...
        <table className="min-w-full text-xs">
          ...
          <tbody>
            {table.getRowModel().rows.length ? (
              ...
              <tr><td colSpan={columns.length} className="text-center py-4">No types loaded.</td></tr>
            )}
          </tbody>
        </table>
      </div>
