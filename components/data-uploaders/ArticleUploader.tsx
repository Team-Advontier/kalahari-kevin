...
export const ArticleUploader: React.FC = () => {
  ...
  // Table columns
  const columns = React.useMemo<ColumnDef<ArticleRow, any>[]>(
    () => [
      { accessorKey: 'articleid', header: 'Article ID' },
  ...
  const handleExportCSV = () => {
    ...
    saveAs(blob, 'flattened_articles.csv');
  };
  const handleExportXLSX = () => {
    ...
    saveAs(blob, 'flattened_articles.xlsx');
  };
  ...
  return (
    <div className="flex flex-col gap-4 p-4 border rounded-md bg-white shadow w-full max-w-5xl mx-auto mt-8">
      <h2 className="text-xl font-bold mb-2">Upload Articles (Teamwork Desk)</h2>
      <div className="flex gap-4 items-center">
        ...
        <Button onClick={handleExportCSV} variant="outline" disabled={articles.length === 0}>Export CSV</Button>
        <Button onClick={handleExportXLSX} variant="outline" disabled={articles.length === 0}>Export XLSX</Button>
        <Button onClick={handleInsert} variant="default" disabled={uploading || articles.length === 0}>
          {uploading ? 'Uploading...' : 'Insert to Supabase'}
        </Button>
      </div>
      <div className="flex items-center gap-4 text-sm">
        <span>Rows: <b>{articles.length}</b></span>
        {results.length > 0 && (
        ...
        <table className="min-w-full text-xs">
          ...
          <tbody>
            {table.getRowModel().rows.length ? (
              ...
              <tr><td colSpan={columns.length} className="text-center py-4">No articles loaded.</td></tr>
            )}
          </tbody>
        </table>
      </div>
